...
export function AIAssistant({ isOpen, onToggle }: AIAssistantProps) {
  ...
  const generateResponse = (userInput: string): string => {
    ...
    if (input.includes('deviation') || input.includes('quality')) {
      ...
      return "I'm your AI assistant. How can I help you with your quality management system today?";
    }
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 font-sans">
      {isOpen && (
        <div className="w-[350px] h-[450px] bg-card rounded-2xl shadow-2xl border border-border flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-200">
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-card">
            ...
            <button
              onClick={onToggle}
              className="text-muted-foreground hover:text-foreground hover:bg-muted p-1.5 rounded-lg transition-all"
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-muted/30">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-card border border-border rounded-xl rounded-bl-none px-4 py-3 shadow-sm">
          ...
          <div className="px-4 py-3 border-t border-border bg-card">
            <div className="flex gap-2">
              <input
                ...
                disabled={isLoading}
                className="flex-1 px-4 py-2.5 bg-muted border border-border rounded-lg text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50"
              />
              <Button
