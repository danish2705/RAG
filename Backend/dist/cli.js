...
async function main() {
    ...
    const approvedClassification = classificationResult.stages.classification.parsed;
    console.log("\n--- Stage 2: Impact Assessment (simulating human approval) ---");
    const impactResult = await runImpactAssessmentOnly(query, contextText, approvedClassification);
    ...
    const approvedImpactAssessment = impactResult.stages.impactAssessment.parsed;
    console.log("\n--- Stage 3: RCA (simulating human approval) ---");
    const rcaResult = await runRCAOnly(query, contextText, approvedClassification, approvedImpactAssessment);
    ...
    const capaResult = await runCAPAOnly(query, approvedClassification, approvedImpactAssessment, approvedRCA);
