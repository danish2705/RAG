    ...
    <DialogContent className="sm:max-w-md">
      ...
      <div className="py-4 space-y-3">
        <p className="text-sm text-muted-foreground">
          Please enter your name to record who is saving this deviation case.
        </p>
...
export const SummaryImpactCard: React.FC<{ entry: any; provenance: any }> = ({
  entry,
  provenance,
}) => {
    ...
    <Card className="shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-lg">
          {entry.category}
          {(keyProv?.severity?.source === "modified" ||
            keyProv?.rationale?.source === "modified") && (
...
export const SummaryClassificationCard: React.FC<{
  parsed: any;
  provenance: any;
}> = ({ parsed, provenance }) => (
  <Card>
    <CardHeader>
      <CardTitle className="flex items-center gap-2">
        <Sparkles className="h-5 w-5 text-blue-600" /> Classification
      </CardTitle>
    </CardHeader>
    <CardContent className="space-y-6">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Badge className={getClassificationBadgeClass(parsed.classification)}>
          {parsed.classification}
        </Badge>
        <SummaryModifiedBadge
...
export const SummaryRcaSection: React.FC<{ parsed: any; provenance: any }> = ({
  parsed,
  provenance,
}) => (
  <>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          Contributing Factors <Sparkles className="h-5 w-5 text-blue-600" />
        </CardTitle>
      </CardHeader>
...
export const SummaryCapaSection: React.FC<{
  parsed: any;
  provenance: any;
  correction?: string;
}> = ({ parsed, provenance, correction }) => (
  <>
    {correction && (
      <Card>
        <CardHeader>
