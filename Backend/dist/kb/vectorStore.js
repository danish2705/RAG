...
function tableFor(source) {
    ...
    if (!table) {
        throw new Error(`Unknown source "${source}" - no chunks table mapped for it. ` +
...
export async function buildIndex(chunks) {
    ...
    const existingResult = await pool.query(`SELECT doc_key, chunk_index, content_hash FROM ${table}`);
    const existingHashes = new Map(); // "docKey::index" -> hash
    ...
    if (pending.length > 0) {
        ...
        pending.forEach((row, idx) => {
            ...
            params.push(row.docKey, row.chunkIndex, row.hash, row.text, toSqlVector(vectors[idx]));
        });
        await pool.query(`INSERT INTO ${table} (doc_key, chunk_index, content_hash, text, embedding)
...
export async function retrieve(query, index, topK = 3) {
    const table = tableFor(index.source);
    const [queryVector] = await embedTexts([query]);
    const result = await pool.query(`SELECT text, doc_key
     FROM ${table}
     ORDER BY embedding <=> $1
     LIMIT $2`, [toSqlVector(queryVector), topK]);
